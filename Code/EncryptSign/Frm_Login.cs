...
namespace EncryptSign
{
    public partial class Frm_Login : Form
    {
        ...
        private void XR_Button_Login_Click(object sender, EventArgs e)
        {
            ...
            string usernametxt = this.XR_TextBox_User.Text;
            string pwdtxt = this.XR_TextBox_Pass.Text;

            bool loginflag = true;

            if ((!String.IsNullOrWhiteSpace(usernametxt)) && (!String.IsNullOrWhiteSpace(pwdtxt)))
            {
                Hashtable ht = new Hashtable();
                ht.Add("Uid", usernametxt.Trim());
                //  ht.Add("PwdHash", EncryptSign.Handler.CommonHandle.sha256_hash(pwdtxt.Trim()));
                User loginUser = BaseDAL.Get<User>("GetUserInfo", ht);
                if (loginUser == null)
                {
                    loginflag = false;
                    XtraMessageBox.Show("邮箱未注册", "登录失败", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    if (loginUser.PwdHash == EncryptSign.Handler.CommonHandle.sha256_hash(pwdtxt.Trim()))
                    {
                        LoginUser.ID = loginUser.ID;
                        LoginUser.Uid = loginUser.Uid;
                        LoginUser.PublicKey = loginUser.PublicKey;
                        LoginUser.PrivateKey = loginUser.PrivateKey; 
                        LoginUser.Name = loginUser.Name;
                        EncryptFrm ecfrm = new EncryptFrm();
                        ecfrm.StartPosition = FormStartPosition.CenterScreen;
                        ecfrm.Show();
                        this.Hide();
                    ...
                    {
                        loginflag = false;
                        XtraMessageBox.Show("用户名或密码错误", "登录失败", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
            else
            {
                loginflag = false;
                XtraMessageBox.Show("请将登录信息输入完整", "登录失败", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        {
            RegisterFrm regfrm = new RegisterFrm(this);
